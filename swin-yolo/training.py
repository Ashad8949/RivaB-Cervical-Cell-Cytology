...
from swin_backbone import SwinTrainer, SwinDetectionModel
...
class ImprovedModelTrainer:
    ...
    def train_fold(self, fold_idx: int, fold_dir: str) -> Optional[Dict]:
        """Train a single fold using SwinTrainer."""
        print(f"\n{'='*80}")
        print(f"TRAINING FOLD {fold_idx} - YOLOv11x-Swin (Transformer Backbone)")
        print(f"{'='*80}")
        
        print(f"  Model YAML: {self.model_config.primary_model}")
        ...
        SwinTrainer.swin_variant = self.model_config.swin_variant
        
        # Build training overrides dict
        overrides = dict(
        ...
        # Train using custom SwinTrainer
        trainer = SwinTrainer(overrides=overrides)
        trainer.train()
    ...
    def _validate_model(
        ...
        # Import swin_backbone ensures SwinDetectionModel is registered for unpickling
...
Examples:
  ...
  python training.py --device 1
    ...
    parser.add_argument('--model', type=str, default='yolo11x.yaml',
                       help='YOLO model YAML config (yolo11x.yaml, yolo11l.yaml, etc.)')
